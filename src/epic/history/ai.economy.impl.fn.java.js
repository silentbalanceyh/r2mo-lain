...
const detectMaven = (modulePath) => {
    const configuration = {};
    // pom.xml 文件检查
    const pom = nodePath.join(modulePath, 'pom.xml');
...
const detectPlugin = (modulePath, configuration = {}) => {
    ...
    // 资源文件目录
    configuration.resourcePlugin = nodePath.join(resource, 'plugin', module);
    configuration.resourceLog = nodePath.join(resource, 'plugin', module, 'annal');

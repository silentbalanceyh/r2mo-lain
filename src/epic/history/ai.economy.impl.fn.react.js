...
const reactReady = (config = {}, files = {}) => {
    ...
    // 资源目录
    const runtime = {};
    runtime.resource = nodePath.join(config.pathRoot, config.pathResource);
    runtime.resourceFiles = {};
    __IO.dirCreate(runtime.resource);
    // 组件目录
    runtime.ui = nodePath.join(config.pathRoot, config.pathUi);
    runtime.uiFiles = {};
    ...
    if (tpl.target) {
        ...
        runtime.resourceFiles[filename] = nodePath.join(runtime.resource, filename + ".json");
        runtime.uiFiles[filename] = nodePath.join(runtime.ui, filename + ".js");
    } else {
        /*
         * 非fileJs模式
         */
        resource.filter(each => undefined !== each).forEach(each => {
            const file = nodePath.join(runtime.resource, each + ".json");
            runtime.resourceFiles[each] = file;
            __LOG.info(`${__STR.strWidth(each)} = ${file.blue}`);
        });
        ui.filter(each => undefined !== each).forEach(each => {
            const file = nodePath.join(runtime.ui, each + ".js");

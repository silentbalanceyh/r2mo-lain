...
1. 从 mdc 规则中检索项目启动相关命令（如 `./dev-build.sh`、`./dev-start.sh`、`npm run dev` 等）。
2. 检测环境是否已经启动：
   - 若已启动，先执行停止操作（`./dev-stop.sh` 或对应停止命令）。
   - 若未启动，继续下一步。
3. 编译最新版（执行 `./dev-build.sh` 或对应构建命令）。
4. 拉起环境（执行 `./dev-start.sh` 或对应启动命令），确保拉起环境的整体稳定性，不出现其他额外流程。

## Commands

1. 读取 `.mdc` 规则文件中 `dev-start` / `dev-build` / `dev-stop` 相关命令
2. 基于启动命令特征（脚本名/路径）检测当前项目进程是否运行 — `pgrep -f "dev-start.sh"` 或从 mdc 中提取的启动命令关键字

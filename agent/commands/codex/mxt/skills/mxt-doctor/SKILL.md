---
name: mxt-doctor
description: Use when the user asks Codex to verify and align anti-drift baseline metadata in .r2mo/doctor/<profile>/; the skill analyzes project characteristics and current state, runs mxt doctor --gen??? for fresh comparison, then intelligently remediates .r2mo/doctor/ configs to match the project's actual structure — fixing misclassifications, removing redundancies, adding missing signals, and aligning profile scope.
---

---
name: mxt-loop
description: Use when the user asks Codex to run a closed-loop task workflow with "$mxt-loop 001"; executes RUN → VERIFY → END → GOON → VERIFY → END_REVIEW cycle with remediation loop on review findings and checkpoint resume.
---
